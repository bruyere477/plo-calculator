...
if not st.session_state.answered:
    ...
    # Auto-submit as timeout if timer runs out
    if remaining == 0:
        process_answer("TIMEOUT")
        st.rerun()

    # Visual HTML Progress Timer Bar
    components.html(f"""
        <div style="font-family: sans-serif; font-weight: bold; margin-bottom: 5px;">
            ⏱️ Time Remaining: <span id="timer" style="color: #ff4b4b;">{remaining}</span>s
        </div>
        <div style="width: 100%; background-color: #333; height: 12px; border-radius: 6px;">
            <div id="bar" style="width: {remaining * 10}%; background-color: #ff4b4b; height: 12px; border-radius: 6px; transition: width 1s linear;"></div>
        </div>
        <script>
            var left = {remaining};
    ...
    # 2x2 Grid of Answer Buttons for Rapid Clicking
...
# Feedback Banner State
    ...
    if is_correct:
        ...
        st.error(f"⏰ **TIME OUT!** Correct answer was **{st.session_state.correct_answer}**")
    else:
        st.error(f"❌ **WRONG!** You picked *{st.session_state.user_choice}*. Correct: **{st.session_state.correct_answer}**")
    ...
    if st.button("Next Hand ➡️ (Space/Click)", use_container_width=True):
        generate_new_question(mode_map[board_mode])
        st.rerun()
