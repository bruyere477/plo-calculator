...
def analyze_plo_straight_draw(hand_str, board_str):
    ...
    for candidate_rank in range(2, 15):
        ...
        # Dynamic board combo size: PLO requires exactly 2 hole cards and 3 board cards
    ...
    # Categorize draws appropriately regardless of Flop (3 cards) or Turn (4 cards)
    if total_outs >= 20:
...
def generate_new_question(board_type="Random"):
    """Generates a random hand, board (flop or turn), correct answer, and choices."""
    suits = ['s', 'h', 'd', 'c']
    deck = [f"{r}{s}" for r in ALL_RANKS for s in suits]

    # Decide board size: 3 for Flop, 4 for Turn
...
board_mode = st.sidebar.selectbox(
    "Street Mode",
    ["Random (Flop or Turn)", "Flop Only", "Turn Only"]
)

# Map sidebar selection to generator parameter
mode_map = {
    "Random (Flop or Turn)": "Random",
    "Flop Only": "Flop Only",
    "Turn Only": "Turn Only"
}
...
# Display current problem street & cards
st.subheader(f"Board ({st.session_state.street})")
...
# Feedback and Explanation Section
    ...
    with st.expander("Show Detailed Hand Analysis", expanded=True):
        st.write(f"- **Street:** {st.session_state.street}")
